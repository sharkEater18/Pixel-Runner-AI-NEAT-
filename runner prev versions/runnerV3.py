...
player_walk_id = 0
player_surface = player_walk_list[player_walk_id]
...
def player_animation():
    global player_surface, player_walk_id
    # Jump
    ...
    # Walk
        ...
        if player_walk_id >= 2: 
            player_walk_id = 0 
        player_surface = player_walk_list[int(player_walk_id)]
...
snail_id = 0
snail_surface = snail_walk_list[snail_id]
...
fly_id = 0
fly_surface = fly_walk_list[fly_id]

# Timer
enemy_timer = pygame.USEREVENT + 1
pygame.time.set_timer(enemy_timer, 1500)

# Movement & Collision Function + Animation
def enemy_movement(enemies):
    global snail_surface, snail_id, fly_surface, fly_id
    if not enemies:
        return []
    ...
    for enemy_rect in enemies:
        ...
        if enemy_rect.bottom == 300:
            ...
            if snail_id >= 2:
                snail_id = 0
            snail_surface = snail_walk_list[int(snail_id)]
        ...
        # Fly
            ...
            if fly_id >= 2:
                fly_id = 0
            fly_surface = fly_walk_list[int(fly_id)]
        ...
        if event.type == enemy_timer and not game_over:
            ...
            if choice_enemy:
                enemy_rect_list.append(
                    snail_surface.get_rect(bottomright=(range_x, 300))
                )
            else:
                enemy_rect_list.append(
                    fly_surface.get_rect(bottomright=(range_x, 190))
                )
